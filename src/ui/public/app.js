...
const TABS = ['overview', 'findings', 'history', 'compare', 'settings', 'funnel'];
...
function render() {
  ...
  const view = { new: viewNew, overview: viewOverview, findings: viewFindings, history: viewHistory, compare: viewCompare, settings: viewSettings, funnel: viewFunnel }[state.view];
...
// ------------------------------------------------------------ funnel
// Monitoring for the free public scanner (a separate service, separate db —
// see src/public/server.js). Not scoped to a client/run, same as Settings.

const pct = (n) => `${Math.round((n ?? 0) * 100)}%`;

function funnelBarRows(rows, key = 'n') {
  const max = Math.max(1, ...rows.map((r) => r[key]));
  // Overrides .bar-row's default 74px label column — funnel labels (device
  // strings, button ids) run longer than the four-letter severity words that
...
async function viewFunnel(el, alive = () => true) {
  el.innerHTML = '<h1>Funnel</h1><div class="skeleton" style="height:220px;margin-top:16px"></div>';
  const [summary, runsData, incidentsData] = await Promise.all([
    api('/api/funnel/summary'), api('/api/funnel/runs?limit=30'), api('/api/funnel/incidents'),
  ]);
  if (!alive()) return;
  const s = summary;

  el.innerHTML = `<h1>Funnel</h1>
    <p class="dim" style="font-size:var(--fs-sm)">Usage of the free public scanner — a separate
      service from your own audits, monitored here because it's the same private dashboard.</p>
    ...
    <div class="row" style="align-items:flex-start">
      <div class="card" style="flex:1 1 260px">
        ...
        <div class="bars">${funnelBarRows(s.devices.map((d) => ({ label: d.device || 'unknown', n: d.n })))}</div>
      </div>
      <div class="card" style="flex:1 1 260px">
        <h2 style="margin-top:0">Region</h2>
        <div class="bars">${funnelBarRows(s.regions.map((r) => ({ label: r.country, n: r.n })))}</div>
      </div>
      <div class="card" style="flex:1 1 260px">
        <h2 style="margin-top:0">Donation clicks</h2>
        <div class="bars">${funnelBarRows(s.clicks.map((c) => ({ label: c.button, n: c.n })))}</div>
      </div>
    </div>
    ...
    <h2>Recent scans <button class="btn" id="funnel-cleanup" style="margin-left:12px">Run 15-day cleanup now</button></h2>
    <div class="card" id="funnel-runs">
      ${runsData.runs.map((r) => `
        <div class="row" style="justify-content:space-between;border-bottom:1px solid var(--line);padding:var(--s-2) 0">
  ...
  $('#funnel-cleanup').addEventListener('click', async () => {
    try {
      const r = await post('/api/funnel/cleanup');
      toast(`Deleted ${r.deleted} run${r.deleted === 1 ? '' : 's'} older than 15 days.`);
      setView('funnel');
    } catch (err) { toast(err.message); }
  });

  $$('#funnel-runs [data-delete]').forEach((btn) => btn.addEventListener('click', async () => {
    ...
    try { await post(`/api/funnel/runs/${encodeURIComponent(id)}/delete`); toast('Deleted'); setView('funnel'); }
    catch (err) { toast(err.message); }
  }));

  $$('#funnel-runs [data-flag]').forEach((btn) => btn.addEventListener('click', async () => {
    const id = btn.dataset.flag;
    const note = await ask({ title: 'Flag this scan', label: 'What looks wrong?', confirmText: 'Flag' });
    if (note == null) return;
    try { await post(`/api/funnel/runs/${encodeURIComponent(id)}/flag`, { note }); toast('Flagged'); setView('funnel'); }
    catch (err) { toast(err.message); }
  }));

  $$('#incidents [data-dismiss]').forEach((btn) => btn.addEventListener('click', async () => {
    try { await post(`/api/funnel/incidents/${encodeURIComponent(btn.dataset.dismiss)}/dismiss`); setView('funnel'); }
    catch (err) { toast(err.message); }
  }));
  ...
  $$('#incidents [data-rule]').forEach((btn) => btn.addEventListener('click', async () => {
    ...
    if (!pattern) return;
    try {
      await post(`/api/funnel/incidents/${encodeURIComponent(btn.dataset.rule)}/rule`, { patternType: 'domain', pattern, action: 'treat_as_blocked' });
      toast('Rule created');
      setView('funnel');
    } catch (err) { toast(err.message); }
  }));
